...
headline: "Guide to the Heat Pump Metrics"
excerpt: "Step-by-step guide to uploading measured values from Bosch CS5800/6800i and Buderus WLW176/186i and comparing them with other systems."
...
This guide shows how to provide measured values from your Bosch or Buderus heat pump for the Metrics platform and compare real performance factors with other systems. The goal is to make reliable practical data for operation, efficiency and optimisation visible.
...
Therefore, please use the platform in a resource-conscious manner:
...
The **yearly overview** shows all months of the selected year.
...
This allows you to see your system in the yearly overview.
Would you also like to see your system in the monthly overview or daily overview?
Then you must provide the measured values every hour.
...
Then you can configure this using the scheduler in your ems-esp gateway.
...
However, if you use Home Assistant, follow the instructions in the next section.
...
You can see your version in the ems-esp interface under `Settings` &rarr; `MQTT` &rarr; `MQTT-Discovery` &rarr; `Entity ID format`. Select the tab that matches your version:
...
To transfer the measured values every hour, you also need an automation, which you can find in Home Assistant under `Settings` &rarr; `Automations & Scenes` &rarr; `Create Automation`.
Select `Create new automation` in the dialog window and `Add Trigger`.
Then select `Time pattern` and enter any number between 0 and 59 under minutes, leaving the other fields blank.
Please choose any number so that not all users upload their measured values at the same time.
Then you must also select `Add Action` and `Perform action`.
