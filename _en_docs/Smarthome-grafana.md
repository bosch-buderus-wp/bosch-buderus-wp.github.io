...
A wide range of chart types, such as line, pie, and bar charts, is available for this purpose.
Data can be integrated via a large number of data sources.
...
Then you also need the Infinity data source.
To do this, select _Data sources_ under _Connections_ in the Grafana menu.
Clicking _+Add new data source_ lets you add the Infinity data source to your Grafana.

This allows you to retrieve and visualize the entities directly via the ems-esp REST interface—without a database:
...
In Grafana, open _Dashboards_, _New_, and _Import_, then paste the contents of the file into the text field.
Then select Infinity as the data source, and you should see the dashboard shown above.
...
The live view uses the so-called [Canvas](https://grafana.com/docs/grafana/latest/panels-visualizations/visualizations/canvas/) to visualize the current measured values at the sensor locations.
Below it, you will see 2 sections.
One contains typical key figures, and above it, in the detailed view, the raw data on which the key figures were calculated.

Key figures:

- **Performance factor**: The total performance factor is the quotient of the total thermal energy generated (heat energy) and the electrical energy used for it.
...
If the share is higher, a faulty installation or configuration could be the cause.
{% endcapture %}
...
If you also want to analyze historical data, it is preferable to install InfluxDB, which is already available as an add-on in most smart home systems ([Home Assistant](https://www.home-assistant.io/integrations/influxdb/), [OpenHAB](https://www.openhab.org/addons/persistence/influxdb/)).
...
Unfortunately, different smart home systems use different storage methods.
...
The InfluxQL queries in the dashboard above are designed for this structure.
If you use a different storage method, you may need to adjust the queries.
...
The following data trends are shown in the chart below:
...
This chart shows the progression of the water temperature in the domestic hot water storage tank over time.
...
The _Sustainability_ section of the dashboard addresses the source of the electrical energy used.
...
To retrieve data from [www.greengrid-compass.eu](https://www.greengrid-compass.eu), you must create a free API key on [api-portal.eco2grid.com](https://api-portal.eco2grid.com/get-started) and enter it in Grafana in the dashboard settings under the variable _api_key_ggc_.

<details>
<summary>Detailed information about the dashboard</summary>
{% capture dashboard-emsesp-influxdb-sustainability-details %} \
**Daily CO<sub>2</sub> emissions of the heat pump vs. a hypothetical gas heating system**

Grid electricity always consists of a mix of various energy sources.
Some of these cause high CO<sub>2</sub> emissions, while others cause very low emissions.
In this chart, the daily electricity consumption is multiplied by the daily average CO<sub>2</sub> emissions per kWh of grid electricity, thereby displaying the daily CO<sub>2</sub> emissions of your heat pump as a bar.
For comparison, the value of a reference heating system, such as a gas heating system, is displayed.
The CO<sub>2</sub> emissions of the gas heating system can be set using the dashboard variable _ref_heating_emissions_.
The default value is 250 gCO<sub>2</sub>eq/kWh.

**Share of renewable energy and daily electrical energy use**

This chart shows the share of renewable energy (solar, wind, water, biomass—no nuclear or fossil energy) during the interval.
The daily energy demand is also shown in the background.
Average values over the entire interval can be found in the legend.
In the image above, the share of renewable energy in March 2025 amounted to 50.2%.
On days with a lot of sunshine, the share of renewable energy is usually higher, while the heat pump's energy consumption is lower (see March 22/23), and vice versa.
Consequently, one might conclude that energy demand behaves oppositely to the share of renewable energy.
Therefore, the legend also shows another average value for renewable energy—this time weighted by the heat pump's daily energy use.
In March, it amounted to 47.1%—slightly below the unweighted average, but only a few percentage points away.
{% endcapture %}
...
More visualizations of heat exchanger efficiency and other analyses will soon be available at [https://github.com/bosch-buderus-wp/grafana-dashboards](https://github.com/bosch-buderus-wp/grafana-dashboards/).
