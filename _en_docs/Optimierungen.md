...
For efficient operation and a long service life of the heat pump, there are several optimizations that can be carried out yourself.
...
Another advantage is that lower temperatures cause fewer mineral salts to crystallize, resulting in fewer limescale deposits that additionally impair efficiency.
...
If, on the other hand, you reduce the charging temperature to 6 K, the water only has to be heated to 54 °C at the end, but domestic hot water production naturally takes correspondingly longer.

### Time of day

As the outdoor temperature rises, the heat pump can extract more energy from the ambient air.
...
On such typical days, domestic hot water production should then fall within the preferred time.
However, if your daily consumption is higher, you should shift the Eco stop temperature upward accordingly.
If your daily consumption is lower, you should adjust the Eco stop temperature so that domestic hot water production occurs every other lunchtime.
...
At this point, I would like to emphasize once again that with water temperatures this low, you must either ensure weekly thermal disinfection or water replacement in all lines within a few days [[bwp](https://www.waermepumpe.de/presse/news/details/kein-erhoehtes-legionellenrisiko-bei-waermepumpen/)].
...
If you own a PV system, you can go one step further and optimize domestic hot water production using excess PV power.
The comfort mode not used in the settings mentioned above is used for this purpose.
For further details on implementation, you can refer, for example, to the instructions for [evcc](https://bosch-buderus-wp.github.io/en/docs/smarthome/evcc).
...
We also experienced this problem suddenly from one day to the next, although it had worked without problems for more than a year beforehand.
...
For a third group, the cause was a missing thermal siphon in the domestic hot water line, which the heating contractor then had to retrofit.
In all three cases, domestic hot water continuously flows through the line and loses heat in the process, which is released to the cold pipes and walls.
...
A domestic hot water temperature of 65 °C and an ambient temperature of 20 °C are generally assumed.
...
<br>
With a domestic hot water temperature of 65 °C in the 277 l tank and an ambient temperature of 20 °C, the following equation gives a temperature drop of around 0.21 K per hour:
...
<br>
\\[
P_{new} = P_{Norm} \cdot \frac{T_{Wasser,neu} - T_{Umgebung}}{T_{Wasser,Norm} - T_{Umgebung}} =
\\]
...
This results in a temperature decrease of 0.11 K per hour in our case:
...
In fact, we have a temperature loss of 0.16 K per hour when we do not draw any water and the circulation pump is not running.
The gravity circulation caused by our blocked check valve resulted in an additional temperature loss of 0.33 K per hour.
This leads to additional costs of just under €100 per year at an electricity price of 32 cents/kWh.
...
Heat pump electricity consumption at a seasonal performance factor of 3:
...
Keeping an eye on the domestic hot water temperature loss can therefore certainly pay off financially.

## Domestic hot water circulation

If water remains in the line for a longer period, it cools down.
To be greeted with hot water immediately when turning on the shower or faucet, so-called circulation lines were installed in many houses in the past.
With the help of this additional line, hot water is circulated from the domestic hot water tank, which is often located in the heating basement, to the other floors.
This means less water is wasted running unnecessarily down the drain until hot water finally reaches the faucet or shower.
That initially sounds sensible as a way of conserving the valuable resource of water.

The problem, however, is that heat energy is lost as the hot water circulates through the lines in the colder masonry.
In winter, one could argue that the house is additionally heated by this and therefore no energy is lost.
...
However, due to the risk of Legionella, this is not a good idea.
One initial optimization is to reduce the operating time of the circulation pump in the heat pump settings.
Alternatively, you can have the circulation pump connected to its own socket.
You then use a programmable plug, e.g. from Shelly, which activates circulation for 5 minutes when needed, e.g. in the morning and evening.
This saves water while simultaneously reducing the energy loss from circulation to a minimum.

If you want, you can go one step further and switch on the plug when domestic hot water is needed using a Shelly button or an Amazon Alexa routine.
You then simply press the button or say “Alexa, shower” to start the circulation pump for 5 minutes.

## Cycling behavior

Using the set [heating curve](/en/docs/einstellungen/#heizkurve), the heat pump determines the required target flow temperature for the current/damped outdoor temperature.
Ideally, the actual flow temperature in the heating circuit (T0) would always follow the target flow temperature exactly.
However, especially during the transitional period with relatively high outdoor temperatures, this is not always possible because [modulation](/en/docs/technischer-aufbau/#modulation) cannot reduce the output arbitrarily far.
Depending on the model, the limit is between 12% and 25% of maximum output.
If too much heat is still produced, the heat pump switches off.
...
However, it should not occur too frequently, because the compressor first has to be brought into operating mode after starting, which reduces efficiency, and every unnecessary start increases wear.
You can view the number of compressor starts either on the control panel in the service menu under `Info` &rarr; `Wärmepumpe` &rarr; `Statistik` or in the app.
...
1. Set the minimum flow temperature:
   ...
   If the minimum flow temperature were set to 28 °C, the yellow line in the image above would not continue to fall at 10:30 a.m.
   As a result, the switch-off threshold would not be reached and the heat pump would continue running.
...
2. Alternatively or additionally, you can try to accept the cycling but reduce the number of cycles by using the [room temperature night setback](/en/docs/einstellungen/#raumtemperatur).
   ...
   After that, the flow temperature will probably have fallen considerably, and the heat pump will need several operating hours without cycling to reach the target flow temperature again.
   Fluctuating room temperatures may reduce comfort.

Of course, it is best to ensure when purchasing the heat pump that it is not oversized, so that modulation is largely sufficient even during the transitional period.

## Heating curve and hydraulic balancing
...
The individual room controls (IRC) or radiator thermostats should not regulate the room temperature by cycling, i.e. by constantly opening and closing the water supply—similar to a car that constantly accelerates to maximum speed and then brakes to a standstill instead of driving at a constant speed.
The aim is therefore to supply the rooms continuously with the minimum necessary water temperature.
The heat pump then only has to heat to the minimum flow temperature currently required to bring all rooms to a comfortable temperature.

This minimum flow temperature and the fine adjustment of the flow rate are achieved through what is known as hydraulic balancing.
The following 6-step guide to hydraulic balancing comes from [Haustechnikdialog.de](https://www.haustechnikdialog.de/SHKwissen/2711/Thermischer-Abgleich):

1. **Prepare the system** \
   In a underfloor heating system, the IRCs should be deactivated or the actuators removed during balancing. All throttling at the heating circuit manifolds is fully opened. For radiators, the thermostatic heads are fully opened or temporarily removed. 
   The aim is for the IRCs/thermostats not to intervene during the measurement phase. Room temperatures should be determined only by the heating curve and flow rate.
2. **Lower the heating curve roughly** \
   ...
   Now lower the heating curve step by step. After each change, wait long enough again; for slow underfloor heating systems, ideally around 48 hours. Continue reducing the heating curve until the first room just fails to reach its desired temperature.
   This room is your reference room. It will not be throttled further during the balancing process.
...
4. **Throttle rooms that are too warm step by step** \
   Start with the room that is warmest relative to the desired temperature. Slightly throttle the flow there and then wait at least 1–2 days again.
   If the room is still too warm afterward, throttle it slightly again. If it becomes too cold, open the flow somewhat again.
5. **Check the heating curve after each throttling round** \
   Throttling one room changes the distribution of heating water in the rest of the system. The reference room may therefore become slightly warmer afterward.
   Check the reference room after each throttling round. If it is too warm, the heating curve can be lowered slightly further. The rooms that have already been adjusted are then checked again and corrected slightly if necessary.
6. **Repeat room by room** \
   ...
   Hydraulic balancing is not a one-time task, but rather a gradual fine adjustment over several cold days or even an entire heating season.
...
A difference of a few degrees is fine, but if the differences are too large, the cold rooms are simply heated by the warm rooms and a higher flow temperature is then required in the warmer room.
...
To do this, simply check in autumn at outdoor temperatures of around 15 °C at which temperature you leave the comfort range in the house.
...
To understand the balancing of flow rates between the [primary circuit](/docs/technischer-aufbau#primärkreis) and [heating circuit](/docs/technischer-aufbau#heizkreis), the control of the circuits must be examined in somewhat more detail.
...
This naturally applies only if the valve position at the radiators or underfloor heating remains unchanged.
...
For better efficiency and stability, the thermostats should therefore always be as open as possible, and the flow rate should be set permanently at the lower valve section or flow regulator as described under [Heating curve and hydraulic balancing](#heizkurve-und-thermischer-abgleich).
...
2. the primary circuit delivers a lower flow rate than the heating circuit draws—then the heating circuit conveys colder water from its return directly back into its flow.
...
The following simulator lets you clearly understand the three circuits (refrigerant circuit, primary circuit and heating circuit).
