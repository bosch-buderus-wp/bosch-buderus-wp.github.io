...
Unfortunately, Bosch/Buderus heat pumps do not offer an official interface for retrieving measured values or making settings.
...
You can then reassemble everything and route the cable through the small gap at the top left between the front panel and the housing.
...
You now need to configure your home Wi-Fi network under [Settings &rarr; Network](http://ems-esp/settings/network/settings).
...
Your heat pump appears with 3 devices in the web interface:
...
You can now get started and view and configure the entities through the web interface.
And via the REST API, you can connect the following extensions:

- [Notifications](/en/docs/smarthome/benachrichtigungen) to be informed about status changes by push notification on your smartphone
- Connect energy management systems such as [evcc](/en/docs/smarthome/evcc)
- Use [Gen-AI](/en/docs/smarthome/ai) applications such as Anthropic Claude to analyze your entities.
...
For this, you need an MQTT broker such as [Mosquitto](https://mosquitto.org/), which is already supplied as an optional extension in many smart home systems.
...
To make _ems-esp_ send the measured values to the MQTT broker, you must enable this under _Settings &rarr; MQTT&nbsp;Settings_ and enter the _Broker Address_, _Username_ and _Password_.
You should also enable _Enable MQTT Discovery_; otherwise, you will have to create all entities manually.
