...
Unfortunately, the Bosch/Buderus heat pumps do not offer an official interface for retrieving measured values or changing settings.

Fortunately, there is the open-source project [ems-esp](https://emsesp.org).
If you do not want to build the hardware yourself, you can obtain hardware already flashed with _ems-esp_ from [BBQKees](https://bbqkees-electronics.nl/?lang=de).
I opted for the [ BBQKees Gateway E32 V2.2](https://bbqkees-electronics.nl/product/gateway-e32-v2-ethernet-wifi-ausgabe-v2-kit/?lang=de).
...
You can then reassemble everything and route the cable through the small opening at the top left between the front panel and the housing.
...
When you open the address [http://192.168.4.1](http://192.168.4.1) in your browser, the _ems-esp_ web interface appears.
The default credentials are _'admin'_ with the password _'admin'_.

[![Web interface of _ems-esp_](/assets/images/EMS-ESP.png "ems-esp web interface")](/assets/images/EMS-ESP.png)

Now you still need to set up your home Wi-Fi network under [Settings &rarr; Network](http://ems-esp/settings/network/settings).
...
- **XCU_THH/CS\*800i, Logatherm WLW\*** (Boiler): the heat pump controller \
  The boiler controls all important functions of the heat pump and provides most of the entities.
...
- [Notifications](/en/docs/smarthome/benachrichtigungen) to be informed about status changes via push notification on your smartphone
...
To make _ems-esp_ send the measured values to the MQTT broker, you must activate this under _Settings &rarr; MQTT&nbsp;Settings_ and enter the _Broker Address_, as well as the _Username_ and _Password_.
You should also activate _Enable MQTT Discovery_, because otherwise you will have to create all entities manually.
