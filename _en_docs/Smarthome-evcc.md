...
_evcc_ was originally designed for PV surplus charging of electric vehicles.
...
The standardized SG Ready interface offers 4 functions:

1. Utility lockout period (Ext1=1 / Ext4=0): not yet supported by _evcc_
2. Normal operation (Ext1=0 / Ext4=0): normal operation
3. Boosted operation (Ext1=0 / Ext4=1): increases the room temperature setpoint and/or the domestic hot water setpoint, depending on what was configured under `System settings` &rarr; `Photovoltaic system`.
4. Forced boosted operation (Ext1=1 / Ext4=1): not yet supported by _evcc_

The SG Ready functions are actually implemented using corresponding control lines.
For control via _evcc_, you do not need control lines, but instead use a trick described in more detail [here](https://bbqkees-electronics.nl/2024/10/03/using-the-smart-grid-sg-and-photovoltaic-pv-function-of-your-heat-pump-with-the-ems-gateways/).
The idea is to invert _External input 4_ with `curl -d '{ "value" : "1xxxxxxxxxxx" }' https://ems-esp/api/boiler/hpin4opt` in order to activate _Boosted operation_.
...
- `Increase desired temperature when heating`: when _evcc_ signals a PV surplus to the heat pump, the room temperature setpoint is increased by the configured value
- `Increased domestic hot water comfort`: in the event of PV surplus, the domestic hot water is heated to the stop temperature of the [_Comfort_](/en/docs/einstellungen/#modi) operating mode.
- `Decrease desired temperature when cooling`: in the event of PV surplus, the room temperature setpoint is reduced by the configured value
- `Cooling only with PV energy`: cooling operation is activated only when there is a PV surplus
- `Max. power for compressor`: limits the compressor to the configured power in the event of PV surplus
...
For the heat pump to appear on _evcc_'s home screen, you must also add the heat pump under _loadpoints_.
...
- `Off`: Normal operation—no use of the PV surplus
- `PV` and `Min+PV`: When there is a PV surplus, the heat pump is signaled to enter _Boosted operation_
- `Fast`: The heat pump is immediately signaled to enter _Boosted operation_
