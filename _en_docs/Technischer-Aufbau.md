...
The core component is the so-called refrigeration cycle, in which a refrigerant circulates and absorbs and releases heat.
In the **evaporator**, heat from the ambient air is absorbed by the cold refrigerant.
In the **compressor**, the refrigerant is compressed and thereby heated—as happens when a bicycle pump heats up during pumping.
In the **condenser**, the heat is transferred to the water in the heating system, causing the refrigerant to cool down.
The **expansion valve** reduces the pressure of the refrigerant again, causing it to cool down further—as with a deodorant spray that becomes cold when used.
The refrigerant is then heated again in the evaporator by the ambient air, and the cycle is complete.
...
The [GWP index](https://de.wikipedia.org/wiki/Treibhauspotential) indicates how strongly a substance acts as a greenhouse gas over 100 years compared with <sub>CO</sub>2.

Propane evaporates at −42.1 °C.
This means that at typical temperatures during German winters, the refrigerant absorbs heat from the warmer ambient air in the evaporator and becomes gaseous.
The heat pump uses a rotary piston compressor from Hitachi-Highly ([WHP07600/WHP013300](https://de.scribd.com/document/817999055/Hitachi-Highly-Compressors)) as its compressor.
...
Anyone wishing to counteract this should schedule domestic hot water production for the morning or evening (see [Domestic hot water production settings](/en/docs/einstellungen/#warmwasserbereitung)).
Restarting takes place at −17 °C or +42 °C.
...
This means that the buffer tank is integrated via a T-piece and, in the optimal case, the water flows from the primary circuit to the heating circuit without using the branch to the buffer tank. It would be used if either:
...
- the primary circuit has a lower flow rate than the heating circuit draws—in this case, the heating circuit conveys colder water from its return directly back into its flow.

The primary and heating circuits should therefore have the same flow rates.
...
Sometimes more and sometimes less power is required to reach or maintain the target flow temperature.
At low outdoor temperatures, more than one kilowatt of electrical energy must be used to raise the flow temperature from an outdoor temperature of −5 °C to 37 °C, for example.
...
If the flow temperature cooled down again shortly afterward, it would have to start up briefly, only to switch off again shortly afterward.
...
If less heat is required, inverter control reduces the speed, producing less heat and thus preventing cycling.
However, inverter control cannot modulate down indefinitely, and eventually the lower limit is reached and the heat pump has to cycle.
...
The ice obstructs the airflow, impairing efficiency.
The heat pump then automatically starts the defrost cycle.

For the so-called hot-gas defrost cycle, the 4-way valve reverses the flow direction of the hot refrigerant and sends it to the evaporator instead of the condenser.
...
For proper drainage, it is important that the condensate drain is sufficiently dimensioned and remains free of ice.
The condensate tray heater or the associated heating cable is switched on as required to prevent ice formation.
The complete defrost cycle takes approximately 4–7 minutes.
...
At approximately 9–16 minutes, these hot-gas defrost cycles last considerably longer.
In newer software versions (from [9.10.0](/en/docs/sw-versionen/#9100--970)), every fifth defrost cycle is an intensive defrost cycle; in older versions, every tenth defrost cycle is.
...
For evaluation, the **system boundary** is important initially: In Bosch/Buderus systems, the power consumption includes not only the compressor and fan, but also the electronics and control system, crankcase heater, standby consumption and electric backup heater.
...
As a result, Bosch/Buderus systems may show a poorer performance factor on the display even though they are not technically less efficient.
...
Practical measurements of the actual power produce approximately the following values for the indoor and outdoor units together:

| Operating condition | Small outdoor unit (4/5/7) | Large outdoor unit (10/12) |
|---|---:|---:|
...
If one of the _PC0_ or _PC1_ pumps (Grundfos UPM4L (K) LIN) is also running, approximately 10–75 W is added depending on the speed.

The **crankcase heater**, also known as a compressor or oil sump heater, keeps the switched-off compressor warm.
This prevents too much refrigerant from dissolving in the compressor oil and impairing lubrication during the next start.
How often it is activated depends on the software version and temperatures. With newer software versions from [9.6.0 / 9.6.1](https://bosch-buderus-wp.github.io/en/docs/sw-versionen/#960--961), activation was reduced, especially in summer.
...
As a result, power consumption for heating also occurs in summer, which sometimes causes confusion.
...
The reason is that the relatively good performance factor from the reduced heating/domestic-hot-water/cooling operation over time is worsened by standby consumption during periods of inactivity.
Even in small systems with generally low power consumption during operation, the comparatively high system consumption results in a poorer performance factor.
...
Anyone wishing to learn more about the operating principle, design and efficient operation of heat pumps beyond this technical overview will find an accessible introduction for laypeople and aspiring professionals in the book [Everything you need to know about heat pumps](https://link.amazon/B09tnIJmm){: rel="sponsored"} by energy-saving expert Carsten Herbert.

*Affiliate link: I may receive a commission from a purchase. You will not incur any additional costs.*
