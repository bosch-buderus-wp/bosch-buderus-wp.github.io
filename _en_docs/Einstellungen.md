...
The following briefly explains the most important settings, but by no means all of them.
...
This is not particularly important technically.
However, if you still want to reconcile the two values, you can set a correction value under `System settings` &rarr; `Heating/cooling` &rarr; `Heating circuit 1` &rarr; `Heating` &rarr; `Room temperature offset`.
...
And if you do not want to rely on the automatic summer/winter changeover, you can select `Off` during the summer months.
...
- `Outdoor temperature controlled`: the heating curve is defined exclusively by the flow temperature at the minimum outdoor temperature (far right at -14 °C in the image above)
- `Outdoor temperature with base point`: additionally allows the flow temperature to be set at an outdoor temperature of 20 °C, the so-called base point (far left in the image above)
- `Individual room controlled`: the actual room temperature is also taken into account when a room thermostat from the heat pump accessories is connected

### Outdoor temperature damping

For all settings relating to the outdoor temperature, note that it does not affect the interior of the building at the same time.
When it gets colder outside, it takes a while for the cold to affect the rooms.
This inertia is set using the building damping under `System settings` &rarr; `Heating/cooling` &rarr; `System settings` &rarr; `Building type damping` and results in the so-called _damped outdoor temperature_.
...
Use `System settings` &rarr; `Heating/cooling` &rarr; `Heating circuit 1` &rarr; `Summer/winter changeover` &rarr; `Heating operation up to` or `Start screen` &rarr; `Desired temperature` &rarr; `More...` &rarr; `Heating off from` to set the temperature limit from which the heating period should begin, e.g. 15°C.
...
For this purpose, a delay for the transition from winter to summer and from summer to winter can be defined under the following menu items:

- `System settings` &rarr; `Heating/cooling` &rarr; `Heating circuit 1` &rarr; `Summer/winter changeover` &rarr; `Summer operation delay`: if the limit temperature is exceeded for the set period, the heating is switched off
- `System settings` &rarr; `Heating/cooling` &rarr; `Heating circuit 1` &rarr; `Summer/winter changeover` &rarr; `Heating operation delay`: if the limit temperature is undershot for the set period, the heating is switched on
...
`System settings` &rarr; `Heating/cooling` &rarr; `System settings` &rarr; `Min. outdoor temperature`

Here you set the standard outdoor temperature (NAT), i.e. the lowest temperature that has occurred 10 times within 20 years and remained for a period of at least two consecutive days. The standard outdoor temperature can be looked up at the [German Heat Pump Association (BWP)](https://www.waermepumpe.de/normen-technik/klimakarte/).

### Flow temperature (NAT)

`System settings` &rarr; `Heating/cooling` &rarr; `Heating circuit 1` &rarr; `Heating` &rarr; `Heating curve`

Pressing the endpoint selects it. You can then use the arrow keys to set the required flow temperature at the NAT.
...
Domestic hot water production can optionally take place in these 3 modes:
...
The following values can be set individually for each mode under `System settings` &rarr; `Domestic hot water`:
...
- `Auto`: the corresponding mode is selected automatically based on the current time of day.
  The program that should be active at which time of day must be set for this purpose.

[![Time program on the UI-800](/assets/images/UI800-Zeitprogramm.jpg)](/assets/images/UI800-Zeitprogramm.jpg)

The time programs for _Auto_ mode can be set either in the [app](/en/docs/app/) under `Domestic hot water` &rarr; `Calendar bottom right` or on the control panel's start screen (not in the service menu) under `Domestic hot water` &rarr; `More...>` &rarr; `Time program` &rarr; `Edit`.
In the display above, domestic hot water production is off from 21:00-05:00, domestic hot water is heated using Eco+ mode from 05:00-13:00 and 17:00-21:00, and domestic hot water is produced using Eco mode from 13:00-17:00.
...
It can still be used for frost protection and defrost cycles despite the auxiliary heater lock.
...
This sets the maximum output of the electric auxiliary heater when the compressor is not running, for example in emergency operation.

#### Limitation in DHW operation

Value range: 0kW, 3kW, 6kW, 9kW

This setting defines the maximum output of the electric auxiliary heater that may be provided for domestic hot water production.

#### Bival.pkt. Parallelbetr.

The aforementioned temperature limit from which the auxiliary heater should support the heat pump, the so-called bivalent point, is specified in this menu item, e.g. -7°C.

### Heating delay

However, the auxiliary heater is not switched on based on the bivalent point set above, but according to the 'failure to reach' the target flow temperature, set as a temperature difference-time product, e.g. 600 K\*min.
